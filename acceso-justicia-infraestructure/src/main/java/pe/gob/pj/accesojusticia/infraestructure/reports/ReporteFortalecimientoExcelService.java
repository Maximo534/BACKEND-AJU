...
import pe.gob.pj.accesojusticia.domain.model.negocio.FortalecimientoCapacidades.DetalleParticipante;
import pe.gob.pj.accesojusticia.domain.model.negocio.FortalecimientoCapacidades.DetalleTarea;
...
public class ReporteFortalecimientoExcelService {
    ...
    // 1. HOJA RESUMEN
    // ==========================================
    private void generarHojaResumen(Workbook workbook, List<FortalecimientoCapacidades> lista) {
        Sheet sheet = workbook.createSheet("Resumen Fortalecimiento");
        ...
        // --- COLUMNAS GENERALES ---
        String[] headersGen = {"N°", "CÓDIGO", "DISTRITO JUDICIAL", "TIPO EVENTO", "NOMBRE EVENTO",
                "FECHA INICIO", "FECHA FIN", "MODALIDAD", "DOCENTE"};
        ...
        // --- LLENADO DE DATOS ---
        int rowIdx = 2;
        for (FortalecimientoCapacidades item : lista) {
            ...
            crearCelda(row, col++, item.getTipoEvento(), textStyle); // Nombre o ID según lo tengas mapeado
            crearCelda(row, col++, item.getNombreEvento(), textStyle);
            crearCelda(row, col++, item.getFechaInicio() != null ? item.getFechaInicio().toString() : "", textStyle);
            crearCelda(row, col++, item.getFechaFin() != null ? item.getFechaFin().toString() : "", textStyle);
            crearCelda(row, col++, item.getModalidad(), textStyle);
            crearCelda(row, col++, item.getDocenteExpositor(), textStyle);

            // Totales Participantes (Calculados en Mapper)
    ...
    // ==========================================
    private void generarHojaParticipantes(Workbook workbook, List<FortalecimientoCapacidades> lista) {
        Sheet sheet = workbook.createSheet("Detalle Participantes");
        String[] headers = {"CÓDIGO EVENTO", "DISTRITO JUDICIAL", "TIPO PARTICIPANTE", "RANGO EDAD", "FEMENINO", "MASCULINO", "LGTBIQ+"};

        crearFilaCabeceraSimple(sheet, headers);

        int rowIdx = 1;
        for (FortalecimientoCapacidades padre : lista) {
            if (padre.getParticipantes() != null) {
                for (DetalleParticipante hija : padre.getParticipantes()) {
                    Row row = sheet.createRow(rowIdx++);
                    int c = 0;
                    // Datos Padre (Repetidos para contexto)
                    crearCelda(row, c++, padre.getCodigo(), textStyle);
                    crearCelda(row, c++, padre.getDistritoJudicialNombre(), textStyle);

                    // Datos Hija
                    // Usamos la descripción si la mapeamos, sino el ID
                    String tipo = hija.getDescripcionTipoParticipante() != null ?
                            hija.getDescripcionTipoParticipante() :
                            String.valueOf(hija.getTipoParticipanteId());

                    crearCelda(row, c++, tipo, textStyle);
    ...
    // ==========================================
    private void generarHojaTareas(Workbook workbook, List<FortalecimientoCapacidades> lista) {
        Sheet sheet = workbook.createSheet("Detalle Tareas");
        String[] headers = {"CÓDIGO EVENTO", "DISTRITO JUDICIAL", "TAREA REALIZADA", "FECHA"};
