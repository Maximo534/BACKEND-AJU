...
import pe.gob.pj.accesojusticia.infraestructure.db.negocio.repositories.MovJusticiaItineranteRepository;
import pe.gob.pj.accesojusticia.infraestructure.db.negocio.repositories.masters.*;
...
public class ReporteJusticiaItineranteService {
    ...
    public byte[] generarFichaItinerante(Long id) throws Exception {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            Paragraph pDistrito = new Paragraph(nombreCorte != null ? nombreCorte.toUpperCase() : "", FONT_TITULO);
            ...
            PdfPCell cValNum = new PdfPCell(new Phrase(entity.getCodigo(), FONT_NORMAL_8));
            ...
            agregarFila(document, "Distrito", ": " + obtenerNombreDist(entity.getDistritoId())); // Corregido nombre getter si es distinto
            ...
            if (esPais) {
                agregarFila(document, "Si es sí, Mencione el Tambo", ": " + obtenerNombreTambo(entity.getTambo())); // Ojo: Tambo suele ser string en tu entidad
            }
    ...
    private void crearTablaAtendidasMatriz(Document doc, List<MovJiPersonasAtendidasEntity> lista) throws DocumentException {
        ...
        if (lista == null || lista.isEmpty()) {
            ...
            Map<Long, List<MovJiPersonasAtendidasEntity>> porVuln = lista.stream()
                    .collect(Collectors.groupingBy(MovJiPersonasAtendidasEntity::getTipoVulnerabilidadId));

            for (Map.Entry<Long, List<MovJiPersonasAtendidasEntity>> entry : porVuln.entrySet()) {
                ...
                for (String r : rangos) {
                    ...
                    int f = data != null ? num(data.getCantFemenino()) : 0;
                    int m = data != null ? num(data.getCantMasculino()) : 0;
                    int l = data != null ? num(data.getCantLgtbiq()) : 0;
    ...
    // --- HELPERS PARA OBTENER NOMBRES DESDE MAESTRAS (Usan Long) ---
    ...
    // OJO: Si Tambo en entidad es String, este método podría fallar si repo espera Long. Ajusta si es necesario.
    private String obtenerNombreTambo(String nombreTambo) { return nombreTambo; } // Ajustado por si acaso
    private String obtenerNombreMateria(Long id) { return repoMateria.findById(id).map(e -> e.getDescripcion()).orElse(String.valueOf(id)); }
    private String obtenerNombreVulnerabilidad(Long id) { return repoVuln.findById(id).map(e -> e.getDescripcion()).orElse(String.valueOf(id)); }
