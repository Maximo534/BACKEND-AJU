...
import pe.gob.pj.accesojusticia.infraestructure.mappers.JusticiaItineranteMapper;
...
import pe.gob.pj.accesojusticia.infraestructure.rest.responses.JusticiaItineranteResponse;
...
public class GestionJusticiaItineranteController implements GestionJusticiaItinerante, GenerarHttpHeader, MonitorearRequest {

    GestionJusticiaItineranteUseCasePort useCase;
    ...
    @Override
    public ResponseEntity<GlobalResponse> listar(PeticionServicios peticion, int pagina, int tamanio, ListarItineranteRequest filtros) {
    ...
    public ResponseEntity<GlobalResponse> obtenerPorId(PeticionServicios peticion, Long id) {
        JusticiaItinerante encontrado = useCase.buscarPorId(peticion.getCuo(), id);
    ...
    @Override
    public ResponseEntity<GlobalResponse> registrar(PeticionServicios peticion, RegistrarFjiRequest request,
                                                    MultipartFile anexo, List<MultipartFile> videos, List<MultipartFile> fotos) {
        cargarTramaPeticion(peticion, request);

        try {
            JusticiaItinerante dominio = mapper.toDomainRegistrar(request, peticion);

            JusticiaItinerante registrado = useCase.registrar(peticion.getCuo(), dominio, anexo, videos, fotos);
    ...
    @Override
    public ResponseEntity<GlobalResponse> actualizar(PeticionServicios peticion, RegistrarFjiRequest request) {
        cargarTramaPeticion(peticion, request);
        ...
        try {
            ...
            JusticiaItinerante dominio = mapper.toDomainActualizar(request.getId(), request, peticion);

            JusticiaItinerante actualizado = useCase.actualizar(peticion.getCuo(), dominio);
    ...
    public ResponseEntity<byte[]> descargarFichaPdf(PeticionServicios peticion, Long id) {
        try {
            ...
            headers.setContentDispositionFormData("inline", "Itinerante_" + id + ".pdf");

            return new ResponseEntity<>(pdfBytes, headers, HttpStatus.OK);
        } catch (Exception e) {
            log.error("[{}] Error generando PDF", peticion.getCuo(), e);
