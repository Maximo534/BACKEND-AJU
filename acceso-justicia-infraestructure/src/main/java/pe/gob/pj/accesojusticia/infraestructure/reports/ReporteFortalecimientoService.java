...
import pe.gob.pj.accesojusticia.infraestructure.db.negocio.repositories.MovEventoFcRepository;
import pe.gob.pj.accesojusticia.infraestructure.db.negocio.repositories.masters.*;
...
public class ReporteFortalecimientoService {
    ...
    public byte[] generarPdf(Long id) throws Exception {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            // Usamos getCodigo() para mostrar el código formateado (ej: 00001-...)
            ...
            if (entity.getTareasRealizadas() != null && !entity.getTareasRealizadas().isEmpty()) {
                ...
                tareas.stream().map(t -> t.getIndicador().getActividad()).distinct()
                        .forEach(a -> {
                            try { agregarBloqueSimple(document, a.getId() + " " + a.getDescripcion()); } catch (DocumentException e) {}
                        });
                ...
                tareas.stream().map(MaeTareaEntity::getIndicador).distinct()
                        .forEach(i -> {
                            try { agregarBloqueSimple(document, "     " + i.getId() + " " + i.getDescripcion()); } catch (DocumentException e) {}
                        });

                document.add(new Paragraph("b) Tareas Realizadas de la Actividad Operativa: ", FONT_NORMAL_8));
                tareas.forEach(t -> {
                    try { agregarBloqueSimple(document, "          " + t.getId() + " " + t.getDescripcion()); } catch (DocumentException e) {}
                });
            } else {
                agregarEspacio(document);
            ...
            PdfPCell cUsu = new PdfPCell(new Phrase("Registrado por: " + uReg, FONT_NORMAL_8));
    ...
    private void crearMatrizParticipantes(Document doc, List<MovEventoDetalleEntity> lista) throws DocumentException {
        ...
        if (lista == null || lista.isEmpty()) {
            ...
            // Agrupar por Tipo de Participante (ID)
            Map<Long, List<MovEventoDetalleEntity>> agrupado = lista.stream()
                    .collect(Collectors.groupingBy(MovEventoDetalleEntity::getTipoParticipanteId));

            for (Map.Entry<Long, List<MovEventoDetalleEntity>> entry : agrupado.entrySet()) {
                String nombreTipo = obtenerNombreTipoParticipante(entry.getKey());
                agregarCeldaDato(table, nombreTipo, Element.ALIGN_LEFT);

                // Mapear por rango para acceso rápido
                Map<String, MovEventoDetalleEntity> porRango = entry.getValue().stream()
                        .collect(Collectors.toMap(e -> e.getRangoEdad().trim(), x -> x, (p1, p2) -> p1));

                String[] codigosRango = {"01", "02", "03", "04"};
                int totalFila = 0;
    ...
    private String obtenerNombreTipoParticipante(Long id) { return repoTipoPart.findById(id).map(e -> e.getDescripcion()).orElse(String.valueOf(id)); }
