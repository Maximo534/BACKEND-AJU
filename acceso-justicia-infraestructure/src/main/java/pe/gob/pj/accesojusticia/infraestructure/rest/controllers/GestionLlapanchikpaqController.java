...
import pe.gob.pj.accesojusticia.domain.port.usecase.negocio.GestionLlapanchikpaqUseCasePort;
import pe.gob.pj.accesojusticia.infraestructure.mappers.AuditoriaGeneralMapper;
import pe.gob.pj.accesojusticia.infraestructure.mappers.LlapanchikpaqMapper;
...
public class GestionLlapanchikpaqController implements GestionLlapanchikpaq, GenerarHttpHeader, MonitorearRequest {

    GestionLlapanchikpaqUseCasePort useCase;
    ...
    @Override
    public ResponseEntity<GlobalResponse> listar(PeticionServicios peticion, int pagina, int tamanio, ListarLlapanchikpaqRequest filtros) {
    ...
    public ResponseEntity<GlobalResponse> obtenerPorId(PeticionServicios peticion, Long id) {
        LlapanchikpaqJusticia encontrado = useCase.buscarPorId(peticion.getCuo(), id);
    ...
    @Override
    public ResponseEntity<GlobalResponse> registrar(PeticionServicios peticion, RegistrarLlapanchikpaqRequest request,
                                                    MultipartFile anexo, List<MultipartFile> videos, List<MultipartFile> fotos) {
        cargarTramaPeticion(peticion, request);

        try {
            LlapanchikpaqJusticia dominio = mapper.toDomainRegistrar(request, peticion);

            LlapanchikpaqJusticia registrado = useCase.registrar(peticion.getCuo(), dominio, anexo, videos, fotos);
    ...
    @Override
    public ResponseEntity<GlobalResponse> actualizar(PeticionServicios peticion, RegistrarLlapanchikpaqRequest request) {
        cargarTramaPeticion(peticion, request);
        ...
        try {
            ...
            LlapanchikpaqJusticia dominio = mapper.toDomainActualizar(request.getId(), request, peticion);

            LlapanchikpaqJusticia actualizado = useCase.actualizar(peticion.getCuo(), dominio);
    ...
    public ResponseEntity<byte[]> descargarFichaPdf(PeticionServicios peticion, Long id) {
        try {
            ...
            headers.setContentDispositionFormData("inline", "Ficha_Llapanchikpaq_" + id + ".pdf");

            return new ResponseEntity<>(pdfBytes, headers, HttpStatus.OK);
        } catch (Exception e) {
            log.error("[{}] Error generando PDF Llapanchikpaq", peticion.getCuo(), e);
