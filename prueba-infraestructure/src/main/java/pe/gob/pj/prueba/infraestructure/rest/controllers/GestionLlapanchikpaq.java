...
@RestController
@Validated
@RequestMapping(value = "/llapanchikpaq", produces = {MediaType.APPLICATION_JSON_VALUE})
@Tag(name = "GestionLlapanchikpaq", description = "API para la gestión de Llapanchikpaq Justicia")
public interface GestionLlapanchikpaq {

    @GetMapping
    @Operation(summary = "Listar Eventos de Llapanchikpaq")
    ResponseEntity<GlobalResponse> listar(
            ...
            @ModelAttribute ListarLlapanchikpaqRequest filtros
    );
    ...
    ResponseEntity<GlobalResponse> registrar(
            @Parameter(hidden = true) @RequestAttribute(name = ProjectConstants.PETICION) PeticionServicios peticion,
            @Valid @RequestPart("data") RegistrarLlapanchikpaqRequest request,
